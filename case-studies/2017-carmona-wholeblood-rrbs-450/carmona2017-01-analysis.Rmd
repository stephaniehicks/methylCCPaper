                                ...
                                returnAll = FALSE, meanPlot = FALSE, verbose = TRUE, ...) 
{
    ...
    compData <- minfi:::pickCompProbes(mSet = referenceMset, cellTypes = cellTypes, 
...
df.450k.Houseman = tidyr::gather(cbind("samples" = pData(RGset_target450k)$sampleID, 
                                "rep" = c(rep(TRUE, 2), rep(FALSE, 9)), 
                                as.data.frame(counts.450k.Houseman)), 
                          celltype, est, -samples, -rep)
```
...
celltype_specific_dmrs <- granges(output$regions_all)
mcols(celltype_specific_dmrs) <- output$zmat
dat <- methylCC:::.preprocess_estimatecc(BS, 
                region_mat = NULL, init_param_method = "random", 
  ...
  a0init = a1init = sig0init = sig1init = tauinit = NULL
init_param_method = "random"
init_theta <- methylCC:::.initialize_theta(n=n, K=K, alpha0=a0init,
                                  alpha1=a1init, sig0=sig0init,
                                  sig1=sig1init, tau=tauinit)

 # Initial guess for cell composition estimates
x0 = (1-zmat_sub) * init_theta$alpha0 + (zmat_sub) * init_theta$alpha1
solve.QP(Dmat = (t(x0)%*%x0),
             dvec = t(x0)%*%t(t(ymat_sub[,1])), 
             Amat = cbind(rep(1,K), diag(K)),
             bvec = c(1, rep(0, K)), 
             meq = 1)$sol

    
  init_pi_mle <- abs(t(apply(ymat_sub, 2, function(ys){
    solve.QP(Dmat = (t(x0)%*%x0),
             dvec = t(x0)%*%t(t(ys)), 
             Amat = cbind(rep(1,K), diag(K)),
             bvec = c(1, rep(0, K)), 
...
init_step <- methylCC:::.initializeMLEs(init_param_method = init_param_method,
                        n = n, K = K, 
...
est <- estimatecc(object = BS,
                  ...
                  verbose = TRUE, epsilon = 0.01,
                  max_iter = 100, init_param_method = "random")
counts.RRBS.Hicks <- cell_counts(est)
write.csv(counts.RRBS.Hicks,
          file = file.path(dataPath, "ests/rrbs-methylCC-estimatecc"),
          row.names = TRUE)
