...
# save(counts450K, file = file.path(workingDir_blsa, "cellcounts/dataBLSA-minfi-estimateCellCounts.RData"))
load(file.path(workingDir_blsa, "ests/cellcounts/dataBLSA-minfi-estimateCellCounts.RData"))
...
table(regions$regions$cellType, regions$regions$status)
table(regions$regions$cellType, regions$regions$L, 
      regions$regions$dmr_status, regions$regions$status)

p1 <- as.data.frame(mcols(regions$regions)) %>% 
...
p2 <- as.data.frame(mcols(regions$regions)) %>% 
  ggplot(aes(x = dm, y = -log(p.value),
             color = cellType)) +
  geom_point() + 
  geom_hline(yintercept=-log(1e-11)) + 
...
df.Hicks <- tidyr::gather(cbind("samples" = rownames(counts.450k.Hicks), counts.450k.Hicks), celltype, est, -samples)
