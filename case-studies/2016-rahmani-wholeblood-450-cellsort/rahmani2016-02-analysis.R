...
#           file = file.path(workingDir_rahmani2016, 
#                            "ests/450k_minfi-estimateCellCounts.csv"),
#           row.names = TRUE)

counts450KHouseman <- read.csv(file.path(workingDir_rahmani2016, 
                                  "ests/450k_minfi-estimateCellCounts.csv"), 
                               row.names = 1)
...
counts450KHicks <- read.csv(file.path(workingDir_rahmani2016, 
                                      "ests/450k_methylCC-estimatecc.csv"), 
                               row.names = 1)

counts450KHicks$Lymph <- 
  rowSums(counts450KHicks[, colnames(counts450KHicks) %in% 
                            c("CD4T", "CD8T", "NK", "Bcell")])
...
gmat <- dfcombined %>% 
  ...
  facet_grid(model~.) + geom_point(size = 3, aes(color=celltype)) +
...
###### using ONLY CpGs (this is for the reviewer)
# output <- find_dmrs(gr_target = NULL, 
...
# write.csv(counts450KHicks,
#           file = file.path(workingDir_rahmani2016,
#                            "ests/450k_methylCC-estimatecc-only-cpgs.csv"),
#           row.names = TRUE)
...
pdf(file.path(workingDir_rahmani2016, 
              "figs/fig-rahmani2016-ests-only-cpgs.pdf"), width = 6, height = 7)
print(gmat)
dev.off()
