...
2. For a given DNAm sample (1 to `nSamples` e.g. 100), 
  a. Simulate a true cell composition $\pi$. 
  b. Simulate a 450k array sample (Y450k = X450k * pi + err) 
...
dataPath <- "/n/irizarryfs01_backed_up/shicks/projects/methylCC/simStudy"

# Z matrix with 64 regions and 4 cell types: 0 = unmethylated, 1 = methylated
Z = as.matrix(expand.grid(c(0,1), c(0,1), c(0,1)))
Z = apply(Z, 2, function(x){rep(x, each=8)})
IDs = c("Lymph","Mono","Gran")
colnames(Z) <- IDs
K = ncol(Z)
R = nrow(Z)
nSamples = 100 # number of whole blood samples
sdtrue = 0.1
...
simResults <- foreach(kk = 1:N) %dopar% {
  ...
  pi.truth[1,] <- rbeta(nSamples, shape1 = 3, shape2 = 9) # Lymph
  pi.truth[2,] <- rbeta(nSamples, shape1 = 4, shape2 = 39)   # Mono 
  pi.truth[3,] <- rbeta(nSamples, shape1 = 9, shape2 = 5) # Gran
...
boxplot(cbind("Houseman" = house450k.all[,1], "methylCC" = hicks450k.all[,1]), 
        main = "Simulated data from 450k platform", ylim=c(0.02, 0.09), 
...
boxplot(cbind("Houseman" = housebs.all[,1], "methylCC" = hicksbs.all[,1]), 
        main = "Simulated data from RRBS platform", ylim=c(0.02, 0.09), 
        xlab="Cell composition estimation method", 
        ylab = "RMSE", cex.axis = 1.5, cex.lab = 1.5, cex.main = 1.5)
